...
public class PredecessorManager {
    ...
    public boolean hasSamePredecessorsForValidQueries(Event event){
        ...
        for (Integer qid: event.getValidQueries()){
            ArrayList<Event> predsForCurrQuery =getPredecessorsAfterLastGraphletSnapshotForQuery(event, qid);
            if (!predsForFirstQuery.equals(predsForCurrQuery)){
                return false;
            }
        }
        return true;
    ...
    /**
     * get the predecessors in a certain range of gloria.stream
     * @param event
    ...
    public HashMap<Integer, Value> sumPrefixEventValuesForKleeneEventTypeForAllQueries(EventType eventType,
        ...
        //all the queries
        ArrayList<Integer> queries = Utils.getInstance().getTemplate().getQueriesOfEventType(eventType);
