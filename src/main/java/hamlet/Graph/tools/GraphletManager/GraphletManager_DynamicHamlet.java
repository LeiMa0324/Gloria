...
public class GraphletManager_DynamicHamlet extends GraphletManager{
    ...
    public void addGraphlet(Graphlet graphlet){
        ...
        ArrayList<Graphlet> exist = this.graphletHashMap.getOrDefault(graphlet.getEventType().getName(), new ArrayList<Graphlet>());
        exist.add(graphlet);
        this.graphletHashMap.put(graphlet.getEventType().getName(), exist);
...
//        graphlet.printGraphletInfo(burst);
    ...
    public HashMap<Integer, Value> newSplitGraphlets(ArrayList<Event> burst){
        ...
        ArrayList<Integer> validStartQueries = Utils.getInstance().getTemplate().getQueriesStartWithEventType(burst.get(0).getType());
