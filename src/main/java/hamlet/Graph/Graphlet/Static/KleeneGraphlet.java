...
//        printGraphletInfo(this.events);
     ...
     * for a gloria.query qi: if the currrent graphlet has no valid events for that, after the propagation, the graphlet's count for qi = =0
    ...
    protected void VanishingSnapshotProcess(){
        ...
        //if all events are invalid for some gloria.query,
        // but the g-snapshot has the count for the gloria.query, keep it in the graphlet's count
        ...
        for (int qid: this.allqids){
            // if the snapshot has count for gloria.query i, but the graphlet dont have the count for gloria.query i, pass the snapshot's count
