package gloriaOptimizer.graph.pool;

import gloriaOptimizer.graph.Transition.Transition;
import gloriaOptimizer.graph.TransitionOperation.*;
import gloriaOptimizer.graph.node.*;
import lombok.Data;
...
public class Pool_SinglePred extends Pool {
    ...
    public ArrayList<Transition> transitionMultiplication(ArrayList<Transition> transitions, Sset p_sset, Node p_node){
        ...
        if (p_sset.isShared()) {
            ...
            if (isBeneficialToLocalCheck(p_sset)||p_sset.containCheckpoint(this.edge.getLeftEventType().getName())) {
        ...
        if (!p_sset.isShared()){
            ...
            if (p_sset.getQueries().size()>1&&this.edge.getEdgeQueries().size()>1) {
                LocalOperation localOperation = new LocalOperation();
                localOperation.setLeftNode(p_node);
                localOperation.operate(p_sset, this.edge);
                operations.add(localOperation);
