...
import gloria.query.aggregator.AvgAndSumAggregator;
import gloria.query.aggregator.CountAggregator;
import gloria.query.pattern.Pattern;
import gloria.query.predicate.AttributeComparisonPredicate;
import gloria.query.predicate.Predicate;
import gloria.query.predicate.SingleValuePredicate;
...
public class QueryParser {
     ...
     * @return a gloria.query instance
     */
    ...
    private void parseReturnLine(String returnLine){
        ...
        if (!returnString.split("\\s")[1].equals("|")) {
//            // the group by column
...
//                    this.pattern.getEventTypes().get(this.pattern.getKleeneIndex()));
        ...
        String aggEventType = aggFunc.equals("COUNT")? "" : aggString.trim().split("\\(")[1].split("\\)")[0].split("\\.")[0];
    ...
    private Predicate parseSingleWhere(String singleWhereString){
        ArrayList<String> predEventTypes = new ArrayList<>();
        ...
        if (a.find()){
            String leftEventType = a.group(1).trim().split("\\.")[0];
            predEventTypes.add(leftEventType);
            ...
            if (right.length==1){
                ...
                String rightEventType = a.group(3).trim().split("\\.")[0];
                predEventTypes.add(rightEventType);
